let doc = eval('document'),
  ...
  css = `<style id="scanCSS">.sc{white-space:pre;color:#ccc;font:14px monospace;}.sc .s{color:#080;cursor:pointer;text-decoration:underline}.sc .f{color:#088}.sc .r{color:#6f3}.sc .r.f{color:#0ff}.sc .s::before{content:"◉";color:red}.sc .r::before{color:#6f3}</style>`,
  tprint = (html) =>
...
export let main = (ns) => {
  ...
  let s = ['home'],
    ...
    r = { home: 'home' },
    fName = (x) =>
      `<a class="s${f.includes(x) ? ' f' : ''}${ns.hasRootAccess(x) ? ' r' : ''}">${x}</a>`
  let tcommand = (x) => {
  ...
  let addSc = (x = s[0], p1 = ['\n'], o = p1.join('') + fName(x)) => {
    for (let i = 0; i < s.length; i++) {
      if (p[i] != x) continue
      let p2 = p1.slice()
      p2[p2.length - 1] = p2[p2.push(p.slice(i + 1).includes(p[i]) ? '├>' : '└>') - 2]
        .replace('├>', '│ ')
        .replace('└>', '  ')
      o += addSc(s[i], p2)
    }
  ...
  doc
    ...
    .forEach((q) => q.addEventListener('click', tcommand.bind(null, r[q.childNodes[0].nodeValue])))
