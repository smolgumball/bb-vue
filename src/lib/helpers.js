...
export function formatNumber(num, minSignificantFigures = 3, minDecimalPlaces = 1) {
  return num == 0.0
    ? num
    : num.toFixed(
        Math.max(minDecimalPlaces, Math.max(0, minSignificantFigures - Math.ceil(Math.log10(num))))
      )
}
...
export async function runCommand_Custom(
  ...
  return await autoRetry(
    ...
    (temp_pid) => temp_pid !== 0,
    () =>
      `Run command returned no pid. (Insufficient RAM, or bad command?) Destination: ${fileName} Command: ${command}`,
    maxRetries,
    retryDelayMs
  )
}
...
export async function autoRetry(
  ...
  while (maxRetries-- > 0) {
    try {
      ...
      const errorLog = `${fatal ? 'FAIL' : 'WARN'}: (${maxRetries} retries remaining): ${String(
        error
      )}`
...
export async function tryGetBitNodeMultipliers(ns) {
  ...
  try {
    return await getNsDataThroughFile(ns, 'ns.getBitNodeMultipliers()')
  } catch {}
  return null
}
