import { getNsDataThroughFile, disableLogs, scanAllServers } from '/lib/helpers.js'
...
export async function main(ns) {
  ...
  let contractsDictCommand = (command) =>
    `Object.fromEntries(${JSON.stringify(contractsDb)}.map(c => [c.contract, ${command}]))`
  let dictContractTypes = await getNsDataThroughFile(
    ns,
    contractsDictCommand('ns.codingcontract.getContractType(c.contract, c.hostname)'),
