  ...
  for (const contractInfo of contractsDb) {
    ...
    if (answer != null) {
      ...
      if (solvingResult) {
        ...
        ns.tprint(
          `Wrong answer for ${contractInfo.contract} on ${contractInfo.hostname}: ${JSON.stringify(
            answer
          )}`
        )
      }
...
function findAnswer(contract) {
  const codingContractSolution = codingContractTypesMetadata.find(
    (codingContractTypeMetadata) => codingContractTypeMetadata.name === contract.type
  )
